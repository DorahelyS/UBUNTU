# UBUNTU
# TEST TESST 

# DESCRIPTION
Ububtu is a full-stack application using React for fronend and Flask for backend, allowing users to track daily emotions and recieve an 'emotional wrap-up' for insigts on stressors throughout the calendar year to ignite healty discussions creating a positive workplace environnment.

# USER STORIES
- User can login to view their own personal page
- User can log daily emotion
- User can track daily emotion
- User can write a brief journal entry
- STRETCH GOAL: User can interact with other users through a like system

# REACT TREE DIAGRAM
<img width="215" alt="Screen Shot 2024-01-30 at 4 16 49 PM" src="https://github.com/DorahelyS/UBUNTU/assets/142290529/c50b4bab-e322-43cc-81a9-f78e2c8c7f0b">

# SCHEMA
<img width="653" alt="Screen Shot 2024-02-13 at 3 15 10 PM" src="https://github.com/DorahelyS/UBUNTU/assets/142290529/7e373a0a-bf1a-4197-ac1f-aedb8efcf125">

# ROUTES
<img width="674" alt="Screen Shot 2024-01-30 at 4 21 06 PM" src="https://github.com/DorahelyS/UBUNTU/assets/142290529/7de6ace2-0924-498c-bc04-d1d5a385158d">

# WIREFRAME
<img width="269" alt="Screen Shot 2024-01-30 at 4 16 19 PM" src="https://github.com/DorahelyS/UBUNTU/assets/142290529/37999895-fd8d-410a-9851-07b56c86f735">

# STRETCH GOALS
1. Create visual graph for each emotion logged - resembeling a calendar 
2. Fun UI styling
3. Like interactive system

# NEW TECHNOLOGY
Tailwind for css

# TRELLO
<img width="1258" alt="Screen Shot 2024-01-30 at 4 17 26 PM" src="https://github.com/DorahelyS/UBUNTU/assets/142290529/1dfd4b6c-78c6-4a78-bcc8-85b0dc341a21">
